...
/**
 * Recipe Variation Class.
 *
 * The recipe variation class handles recipe variation data.
 *
 * @class       MYC_Recipe_Variation
 * @version     0.1
 * @package     MYC/Classes
 * @category    Class
 * @author      e-cook
 */
class MYC_Recipe_Variation extends MYC_Recipe {

	/** @public int ID of the variation itself. */
	public $variation_id;

	/** @public object Parent Variable recipe object. */
	public $parent;
	 ...
	 * @param array $args Array of the arguments containing parent recipe data
	 */
	public function __construct( $variation, $args = array() ) {
		...
		/* Get main recipe data from parent (args) */
		...
		if ( empty( $this->id ) ) {
			throw new Exception( sprintf( 'No parent recipe set for variation #%d', $this->variation_id ), 422 );
		}

		$this->recipe_type = 'variation';
		$this->parent       = ! empty( $args['parent'] ) ? $args['parent'] : myc_get_recipe( $this->id );
	...
	public function __get( $key ) {
		if ( in_array( $key, array_keys( $this->variation_level_meta_data ) ) ) {
			...
			return $this->variation_data = myc_get_recipe_variation_attributes( $this->variation_id );
	...
	public function get_permalink( $item_object = null ) {
		if ( ! empty( $item_object['variation'] ) ) {
			...
			$data_keys    = array_map( 'myc_variation_attribute_name', wp_list_pluck( $item_object['item_meta_array'], 'key' ) );
	...
	public function add_to_cart_url() {
		...
		return apply_filters( 'myc_recipe_add_to_cart_url', $url, $this );
	}
	...
	public function add_to_cart_text() {
		$text = $this->is_purchasable() && $this->is_in_stock() ? __( 'Add to cart', 'myc' ) : __( 'Read more', 'myc' );

		return apply_filters( 'myc_recipe_add_to_cart_text', $text, $this );
	}
	...
	public function variation_is_visible() {
		...
		return apply_filters( 'myc_variation_is_visible', $visible, $this->variation_id, $this->id, $this );
	}
	...
	public function variation_is_active() {
		return apply_filters( 'myc_variation_is_active', true, $this );
	}

	/**
	 * Returns false if the recipe cannot be bought.
	 * Override abstract method so that: i) Disabled variations are not be purchasable by admins. ii) Enabled variations are not purchasable if the parent recipe is not purchasable.
	 *
	 * @return bool
	 */
	public function is_purchasable() {
		...
		return apply_filters( 'myc_variation_is_purchasable', $purchasable, $this );
	}
	...
	public function get_price_html( $price = '' ) {
		...
		if ( $this->get_price() !== '' ) {
			if ( $this->is_on_sale() ) {
				$price = apply_filters( 'myc_variation_sale_price_html', '<del>' . myc_price( $display_regular_price ) . '</del> <ins>' . myc_price( $display_sale_price ) . '</ins>' . $this->get_price_suffix(), $this );
			} elseif ( $this->get_price() > 0 ) {
				$price = apply_filters( 'myc_variation_price_html', myc_price( $display_price ) . $this->get_price_suffix(), $this );
			} else {
				$price = apply_filters( 'myc_variation_free_price_html', __( 'Free!', 'myc' ), $this );
			}
		} else {
			$price = apply_filters( 'myc_variation_empty_price_html', '', $this );
		}

		return apply_filters( 'myc_get_variation_price_html', $price, $this );
	}

	/**
	 * Gets the main recipe image ID.
	 *
	 * @return int
	 */
	...
	public function get_image( $size = 'shop_thumbnail', $attr = array(), $placeholder = true ) {
		if ( $this->variation_id && has_post_thumbnail( $this->variation_id ) ) {
			...
			$image = myc_placeholder_img( $size );
		} else {
			$image = '';
		}
		return $image;
	...
	/**
	 * Returns whether or not the recipe (or variation) is stock managed.
	 *
	 * @return bool|string Bool if managed at variation level, 'parent' if managed by the parent.
	 */
	public function managing_stock() {
		if ( 'yes' === get_option( 'myc_manage_stock', 'yes' ) ) {
	...
	public function get_stock_quantity() {
		return apply_filters( 'myc_variation_get_stock_quantity', true === $this->managing_stock() ? myc_stock_amount( $this->stock ) : $this->parent->get_stock_quantity(), $this );
	}
	...
	/**
	 * Returns whether or not the recipe is in stock.
	 *
	 * @return bool
	 */
	public function is_in_stock() {
		 ...
		 * instock.
		 *
		 * Check is not required for recipes on backorder since they can be
		 * instock regardless of actual stock quantity.
		 */
		if ( true === $this->managing_stock() && ! $this->backorders_allowed() && $this->get_stock_quantity() <= get_option( 'myc_notify_no_stock_amount' ) ) {
			$status = false;
		}

		return apply_filters( 'myc_variation_is_in_stock', $status );
	}

	/**
	 * Set stock level of the recipe variation.
	 *
	...
	public function set_stock( $amount = null, $mode = 'set' ) {
		...
		if ( ! is_null( $amount ) && true === $this->managing_stock() ) {
			...
			delete_transient( 'myc_recipe_total_stock_' . $this->id . MYC_Cache_Helper::get_transient_version( 'recipe' ) );

			// Stock status
			$this->check_stock_status();

			// Sync the parent
			MYC_Recipe_Variable::sync( $this->id );

			// Trigger action
			do_action( 'myc_variation_set_stock', $this );
	...
	public function set_stock_status( $status ) {
		...
		if ( true === $this->managing_stock() ) {
			if ( ! $this->backorders_allowed() && $this->get_stock_quantity() <= get_option( 'myc_notify_no_stock_amount' ) ) {
				$status = 'outofstock';
			}
		} elseif ( 'parent' === $this->managing_stock() ) {
			if ( ! $this->parent->backorders_allowed() && $this->parent->get_stock_quantity() <= get_option( 'myc_notify_no_stock_amount' ) ) {
				$status = 'outofstock';
			}
		}

		if ( update_post_meta( $this->variation_id, '_stock_status', $status ) ) {
			do_action( 'myc_variation_set_stock_status', $this->variation_id, $status );

			MYC_Recipe_Variable::sync_stock_status( $this->id );
		}
	}

	/**
	 * Reduce stock level of the recipe.
	 *
	...
	/**
	 * Increase stock level of the recipe.
	 *
	...
	/**
	 * Returns whether or not the recipe needs to notify the customer on backorder.
	 *
	 * @return bool
	 */
	...
	/**
	 * Returns whether or not the recipe has enough stock for the order.
	 *
	...
	public function get_shipping_class() {
		if ( ! $this->variation_shipping_class ) {
			$classes = get_the_terms( $this->variation_id, 'recipe_shipping_class' );
	...
	/**
	 * Returns the recipe shipping class ID.
	 *
	 * @return int
	 */
	...
	/**
	 * Get formatted variation data with MYC < 2.4 back compat and proper formatting of text-based attribute names.
	 *
	 * @return string
	 */
	public function get_formatted_variation_attributes( $flat = false ) {
		...
		if ( is_array( $variation_data ) ) {
			...
			foreach ( $attributes as $attribute ) {
				...
				$description_name         = esc_html( myc_attribute_label( $attribute[ 'name' ] ) );
				$description_value        = __( 'Any', 'myc' );
				...
				if ( $attribute[ 'is_taxonomy' ] ) {
					...
					foreach ( $post_terms as $term ) {
						if ( $variation_selected_value === $term->slug ) {
							$description_value = esc_html( apply_filters( 'myc_variation_option_name', $term->name ) );
						}
					}

				} else {

					$options = myc_get_text_attributes( $attribute[ 'value' ] );
					...
					foreach ( $options as $option ) {
						...
						$description_value = esc_html( apply_filters( 'myc_variation_option_name', $option ) );
					}
				}
	...
	/**
	 * Get recipe name with extra details such as SKU, price and attributes. Used within admin.
	 *
	 * @return string Formatted recipe name, including attributes and price
	 */
	public function get_formatted_name() {
		...
		$extra_data           = ' &ndash; ' . $formatted_attributes . ' &ndash; ' . myc_price( $this->get_price() );

		return sprintf( __( '%s &ndash; %s%s', 'myc' ), $identifier, $this->get_title(), $extra_data );
	}

	/**
	 * Get recipe variation description.
	 *
	 * @return string
	 */
